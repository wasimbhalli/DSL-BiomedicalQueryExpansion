...
public class Evaluation {
	...
	final static String goldStdPathTRECGenomic = "resources/trecgen2007.gold.standard.tsv.txt";
	final static String pyScript = "resources/script/trecgen2007_score.py";
	final static String goldStdPathBIOAsq="resources/BioASQ-task2bPhaseA-testset1.docs.concepts.documents.gold.json";
	...
	public static void evaluateResults(String experiment, EvaluationType type) throws IOException {
		...
		{
			evaluateTRECGenomicResults(experiment);
		...
		{
			evaluateBIOAsqResults(experiment);
		}
	...
	private static void evaluateTRECGenomicResults(String experiment) throws IOException
	{
		BufferedReader bfr;
		...
		Process proc = rt.exec("python" + " " + pyScript + " " + goldStdPathTRECGenomic + " " + "resources/queryResults.txt");
		try {
			proc.waitFor();
	...
	public static void evaluateBIOAsqResults(String experiment)
	{
		EvaluatorMainClass eval;
    	eval = new EvaluatorMainClass(goldStdPathBIOAsq, "resources/queryResults.json",2);
